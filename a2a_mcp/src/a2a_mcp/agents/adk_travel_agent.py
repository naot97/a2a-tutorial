...
class TravelAgent(BaseAgent):
    ...
    async def init_agent(self):
        ...
        try:
            ...
            logger.error(f'Failed to connect to MCP server at {config.url}: {e}')
            logger.info('Starting with empty toolset due to MCP connection failure')
            tools = []
        ...
        self.agent = Agent(
            ...
            model=LiteLlm(model="azure/gpt-4.1-mini"),
